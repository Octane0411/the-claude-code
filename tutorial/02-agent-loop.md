...
QueryEngine.submitMessage
  ...
  -> processUserInput
  -> query()
  -> transcript / SDKMessage / result
```

`QueryEngine` 的价值，不是替代 `query()`，而是把下面这些能力包装起来：

- conversation 级消息状态
- system prompt parts 预取与初始化 system message
- transcript 持久化
- SDK message 归一化
...
在 `handlePromptSubmit()` 里，直接输入最终也会被包装成一个 `QueuedCommand`，再统一交给 `executeUserInput()`。也就是说：

- “用户此刻刚提交的输入”
- “前面已经排队、现在轮到执行的输入”

这两种来源，最终并不会走两套不同逻辑，而是会在真正进入 `processUserInput()` 之前就被归一化。
...
当 `shouldQuery === true` 时，`processUserInput()` 后面还会执行 `UserPromptSubmit` hooks。这里可能发生几类事情：

- 直接阻断本轮继续
- 注入额外上下文 attachment
- 生成一条“operation stopped by hook”类型的补充消息

所以从运行时角度看，`processUserInput()` 更像：

- 输入规范化层
- 本地命令 / 模式分发层
- 进入主 loop 之前的最后一个 hook gate
...
`query()` 不只是一个 async generator，它最后还会返回一个 `Terminal`。这个返回值里会带上这次 turn 为什么结束，例如：
...
这说明 Claude Code 不只是“往外流消息”，还会把 turn 的结束语义作为 runtime 结果显式返回。
...
尤其是 `transition` 很说明问题。

它不是装饰性字段，而是在每次 `continue` 之前显式记录“为什么会进入下一轮”，例如：
...
- 真正的继续信号，是流式阶段是否收到了 `tool_use block`
...
### recoverable error 不一定立刻显示给用户

还有一个很容易忽略的实现细节：

在流式阶段，`query()` 会暂时压住几类“有可能恢复”的 assistant API error，不立刻 yield 给外层，例如：
...
- 如果后面还有恢复路径，那这些错误不应该先污染 transcript
- 只有恢复失败时，它们才应该真正浮到用户可见层

这使得 Claude Code 的错误处理更像“延迟决策”，而不是“看到错误立刻报错”。
...
### fallback model 也发生在同一个 turn 里

如果流式调用中触发了 `FallbackTriggeredError`，`query()` 不会把这一轮简单判死，而是会：

- tombstone 掉当前已流出的 orphan assistant messages
- 丢弃这轮失败尝试的 pending tool results
- 把 `currentModel` 切到 `fallbackModel`
- 用同一份 turn state 重新采样

这也再次说明，Claude Code 的 turn 是“事务级执行过程”，不是单次请求。
...
从 `query.ts` 的控制流看，大致是：

1. 先处理 prompt-too-long / media-size 这类 recoverable API error
2. 再处理 `max_output_tokens`
3. 再决定是否跑 stop hooks
4. 最后才看 token budget 是否还允许继续

也就是说，“是否继续”是一个严格排序过的 runtime 判定过程。
...
并且工具阶段本身也可能改变下一轮状态，例如：

- 某些工具会返回 `newContext`
- 某些 hook attachment 会要求 `prevent continuation`
- Claude Code 会异步预生成下一轮要显示的 `tool use summary`
...
- `queuedCommandsSnapshot` 不是把所有队列内容一把塞进去，它会过滤 slash commands，并按 main thread / subagent 的 `agentId` 做作用域隔离
- 在进入下一轮前，`query()` 还会调用 `refreshTools()`，让新连上的 MCP tools 能在下一轮立即可见

所以 Claude Code 的工具池和上下文池，都是“每轮重算”的，而不是一次 query 固定到底。
...
在真正 `continue` 之前，它还会额外检查一件事：

- `maxTurns` 是否已经达到上限

只有没超限，才会再次回到 `while (true)` 顶部。

这正是整套 agent loop 的本质：

**不是递归调用模型，而是持续更新一份运行时状态，然后让状态驱动下一轮决策。**

## QueryEngine 比 REPL 额外做了什么

前面我们一直强调 `query()` 是共享核心，但这不代表 `QueryEngine` 只是薄薄一层壳。

从 `src/QueryEngine.ts` 看，它至少还承担了这些 headless / SDK 语义：

- 先调用 `fetchSystemPromptParts()` 组装默认 system prompt、user context、system context
- 在进入 `query()` 前，就把接受到的 user messages 写入 transcript，保证中途被杀掉也能 `resume`
- 包装 `canUseTool()`，把 permission denials 记录成 SDK 结果的一部分
- 先产出一条 system init message，把 model、tools、commands、skills、plugins 等初始化态告诉 SDK 消费方
- 在 `for await (const message of query(...))` 外层，把 assistant / user / compact boundary / tool summaries 重新包装成 SDKMessage

所以 REPL 和 SDK 的真实关系不是：
...
- 它们共享同一个 query runtime
- 但各自有自己的“消息存储、结果语义、对外协议”包装层
